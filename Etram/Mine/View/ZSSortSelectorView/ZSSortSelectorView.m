...
- (void)setDataArr:(NSArray *)dataArr {
    ...
    _mainView.contentSize = CGSizeMake(_dataArr.count*(_width)/3, _height);
    for (int i=0; i<_dataArr.count; i++) {
        ZSScrollButton *btn = [[ZSScrollButton alloc]initWithFrame:CGRectMake(i*(_width)/3, 0, (_width)/3, _height) miantitle:_dataArr[i] count:[NSString stringWithFormat:@"33"]];
        btn.tag = i+33;
        if (!i) {
